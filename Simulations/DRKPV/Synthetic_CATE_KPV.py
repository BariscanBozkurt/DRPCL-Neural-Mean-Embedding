...
from src.simulation_utils.data_generating_processes import generate_synthetic_CATE_data
...
from src.torch_utils.kernel_utils import RBF, ColumnwiseRBF, BinaryKernel
from src.causal_learning.kernel_proxy_causal_learning import KernelProxyVariableHeterogeneousResponse
...
csv_name_for_results = f"../Results/CATE_Synthetic_Experiment/KPV_Synthetic_CATE_Results_{data_sizes_str}_{current_date}.csv"

print(f"Running simulation for sizes: {data_size_list}")

print("Running the script Synthetic_CATE_KPV to generate results and save at {}".format(csv_name_for_results))
...
for n_plus_m in data_size_list:
    for seed in seed_list:
        ...
        V_test_tensor = torch.tensor(covariate_v_test, dtype=torch.float32).to(device)
        ...
        V_tensor_transformed = transformers[4].fit_transform(V_tensor)
        A_test_tensor_transformed = transformers[0].transform(A_test_tensor)
        V_test_tensor_transformed = transformers[4].transform(V_test_tensor)


        outcome_bridge_kpv_algo_param_dict_default = {
                                                    "algorithm_name" : "Kernel_Proxy_Variable",
                                                    "kernel_A" : BinaryKernel(use_length_scale_heuristic = True,).to(device),
                                                    "kernel_W" : RBF(use_length_scale_heuristic = True).to(device),
                                                    "kernel_Z" : RBF(use_length_scale_heuristic = True).to(device),
                                                    "kernel_V" : RBF(use_length_scale_heuristic = True).to(device), 
                                                    "kernel_X" : RBF(use_length_scale_heuristic = True).to(device),      
                                                    "lambda1_" : 0.1,
                                                    "lambda2_" : 0.1,
        ...
        model = KernelProxyVariableHeterogeneousResponse(**outcome_bridge_kpv_algo_param_dict_default,
                                        device = device
                                            )

        model.fit((A_tensor_transformed, Z_tensor_transformed, W_tensor_transformed, V_tensor_transformed), Y_tensor_transformed)

        f_struct_pred = model.predict(A_test_tensor_transformed, V_test_tensor_transformed, transformers[1]).reshape(-1, 1).detach().cpu().numpy()
