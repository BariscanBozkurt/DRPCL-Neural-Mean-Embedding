...
from neural_causal_learning.proxy_outcome_neural_mean_embedding_w_SGD import (HeterogeneousOutcomeBridgePCLNET, 
...
from neural_causal_learning.proxy_treatment_neural_mean_embedding_w_SGD import HeterogeneousTreatmentBridgePCLNET, train_treatment_pcl_net_ate_model
...
def serialize_array_for_csv(x):
    """
    Converts numpy arrays / torch tensors / lists to a JSON string
    so that they can be safely stored in a CSV cell and later recovered.
    """
...
csv_name_for_results = f"../Results/CATE_Synthetic_Experiment/DRPCLNET_SyntheticCATE_Misspecified_Noisy_Proxies_Results_with_SGD_and_log_cosh_{current_date}.csv"

print(f"Running simulation for sizes: {data_size_list}")

print("Running the script Synthetic_CATE_Experiment_with_SGD to generate results and save at {}".format(csv_name_for_results))
...
n_plus_m = 2000
scenario_list = ["treatment_misspecified", "outcome_misspecified", "both_misspecified"]
...
for scenario_ in scenario_list:
    for seed in seed_list:
        ...
        U, W, Z, V, A, Y, covariate_v_test, do_A, EY_do_A = generate_synthetic_CATE_data(   n_sample, 
                                                                                            ...
                                                                                            seed = seed)
        if scenario_ == "treatment_misspecified":
            W = np.abs(W) + np.random.randn(*W.shape) * 100
        elif scenario_ == "outcome_misspecified":
            Z = np.abs(Z) + np.random.randn(*Z.shape) * 100
        elif scenario_ == "both_misspecified":
            W = np.abs(W) + np.random.randn(*W.shape) * 100
            Z = np.abs(Z) + np.random.randn(*Z.shape) * 100
        ...
        Result_Dict = {
                        ...
                        "Estimated_Response" : serialize_array_for_csv(f_struct_pred_om),
        ...
        Result_Dict = {
                        ...
                        "Estimated_Response" : serialize_array_for_csv(f_struct_pred_tm),
                        "Treatment_Grid" : serialize_array_for_csv(do_A),
                        "Ground_Truth_Response" : serialize_array_for_csv(EY_do_A),
                        **meta_params # Merges all hyperparameters into this row
                    }
        ...
        Result_Dict = {
                        ...
                        "Estimated_Response" : serialize_array_for_csv(f_struct_pred_dr),
                        "Treatment_Grid" : serialize_array_for_csv(do_A),
                        "Ground_Truth_Response" : serialize_array_for_csv(EY_do_A),
                        **meta_params # Merges all hyperparameters into this row
                    }
