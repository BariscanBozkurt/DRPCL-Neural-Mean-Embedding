...
# Utilities
from simulation_utils.data_generating_processes import dSprite_ProxyVariable_DatasetV2
...
from torch_utils.densratio import HighDimCausalDensityRatioEstimator, CausalRuLSIFTorch, CausalKDEDensRatioTorch, CausalKLIEPDensRatio
from torch_utils.networks import ConditionalMeanMLP, EnsembleConditionalMeanMLP, ConvBetaVAE_dSprite, train_beta_vae
...
from simulation_utils.treatment_pcl_net_nn_structures import build_nets_for_treatment_pcl_net_dsprite_experiment
...
from neural_causal_learning.proxy_treatment_neural_mean_embedding import TreatmentBridgePCLNET, train_treatment_pcl_net_ate_model

# Outcome Bridge Network Imports
from simulation_utils.outcome_pcl_net_configs import OutcomePCLConfig
from simulation_utils.outcome_pcl_net_nn_structures import build_nets_for_outcome_pcl_net_dsprite_experiment
...
from neural_causal_learning.proxy_outcome_neural_mean_embedding import (OutcomeBridgePCLNET, 
...
csv_name_for_results = f"../Results/dSprite_Experiment/DRPCLNET_dsprite_Results_with_mseClosedForm_{data_sizes_str}_{current_date}.csv"

print(f"Running simulation for sizes: {data_size_list}")

print("Running the script dSprite_Experiment_w_ClosedForm_and_kliep to generate results and save at {}".format(csv_name_for_results))
...
for n_plus_m in data_size_list:
    for seed in seed_list:
        ...
        latent_dim = 16
        target_beta = 1.0 
        betaVAE_lr = 1e-3 if n_sample <= 5000 else 5e-4
        betaVAE = train_beta_vae(A_tensor, latent_dim=latent_dim, target_beta=target_beta, lr = betaVAE_lr, device=device)

        # Extract Latent Features
        A_tensor_compressed = betaVAE.get_latent_features(A_tensor.to(device)).detach()
        W_tensor_compressed = betaVAE.get_latent_features(W_tensor.to(device)).detach()
        ...
        transformers = [TorchIdentityTransformer(), TorchStandardScaler(), TorchStandardScaler(), TorchIdentityTransformer()]
        dens_ratio_transformer = TorchIdentityTransformer()

        ratio_estimator = CausalKLIEPDensRatio(sigmas=[0.5, 3, 5], num_params= [0.2], verbose=True, device = device)
        ratio_estimator.fit(A_tensor_compressed, W_tensor_compressed)
        dens_ratio_tensor = ratio_estimator.predict_ratio(A_tensor_compressed, W_tensor_compressed).to("cpu")
        ...
        print("Ratio of inlier data picked according to density ratio estimation is {}".format(n_sample / A_tensor_compressed.shape[0]))
        ...
        treatment_cfg = TreatmentPCLConfig.dsprite()
        treatment_cfg.plot_loss = False
        treatment_cfg.device = device
        treatment_cfg.reg_second_stage = (10., 100.)
        treatment_cfg.lr_first_stage_base=1e-4
        treatment_cfg.lr_second_stage_ax=1e-4
        treatment_cfg.lr_second_stage_z=1e-4
        treatment_cfg.n_epohcs = 100
        treatment_cfg.weight_decay = 1e-5
        treatment_cfg.reg_first_stage = (1e-3, 1e-1)
        treatment_cfg.reg_second_stage_first = (1e-3, 1e-1)
        ...
        if n_plus_m <= 10000:
            third_stage_lr = 1e-4
        elif (n_plus_m <= 20000) and (n_plus_m > 10000):
            third_stage_lr = 5e-5
        else:
            third_stage_lr = 1e-5
        ...
        outcome_proxy_featurizer,) = build_nets_for_outcome_pcl_net_dsprite_experiment(device = outcome_cfg.device)
        ...
        treatment_proxy_featurizer) = build_nets_for_treatment_pcl_net_dsprite_experiment(device = treatment_cfg.device)
        ...
        del outcome_model, treatment_model, third_stage_net, third_stage_net_dr, third_stage_dataset, third_stage_dataset_val, third_stage_dataset_dr, third_stage_dataset_dr_val
        del first_stage_train_dataset, second_stage_train_dataset, validation_dataset
